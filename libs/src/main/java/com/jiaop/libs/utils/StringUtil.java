...
public class StringUtil {
    ...
    public static String byte2FitSize(long byteNum) {
        if (byteNum < 0) {
            ...
            return String.format(Locale.getDefault(), "%.3fKB", (double) byteNum / ConstUtil.KB);
        } else if (byteNum < ConstUtil.GB) {
            return String.format(Locale.getDefault(), "%.3fMB", (double) byteNum / ConstUtil.MB);
        } else {
            return String.format(Locale.getDefault(), "%.3fGB", (double) byteNum / ConstUtil.GB);
        }
    }
    ...
    public static String utf8Encode(String str) {
        if (!isNullString(str) && str.getBytes().length != str.length()) {
            try {
